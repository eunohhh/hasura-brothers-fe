-- PostgreSQL schema generated from existing MySQL DDL and JPA entities
-- Notes:
-- - MySQL `binary(16)` UUID columns -> uuid
-- - `datetime` -> timestamptz
-- - `tinyint(1)` booleans -> boolean
-- - `json` -> jsonb
-- - auto_increment -> generated always as identity
-- - backticked identifiers removed; reserved words quoted when needed
-- - Default schema set to `public` for Hasura compatibility
...
create table if not exists "user" (
    ...
    meta_thumbnail_url varchar(255),
...
create unique index if not exists invitation_id_idx on invitation (id);
...
create unique index if not exists invitation_owner_id_idx on invitation_owner (id);
...
create unique index if not exists invitation_editor_id_idx on invitation_editor (id);
...
create unique index if not exists widget_id_idx on widget (id);
...
create unique index if not exists invitation_attachment_id_idx on invitation_attachment (id);
...
create unique index if not exists invitation_share_id_idx on invitation_share (id);
...
create unique index if not exists order_id_idx on "order" (id);
...
create unique index if not exists invitation_comment_id_idx on invitation_comment (id);
...
create unique index if not exists coupon_id_idx on coupon (id);
...
create unique index if not exists invitation_rsvp_answer_id_idx on invitation_rsvp_answer (id);
...
-- Additional indexes for FK columns missing explicit indexes
